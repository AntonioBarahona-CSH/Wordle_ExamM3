...
def show_play_again_button():
    play_again_button = pygame.Rect(WIDTH // 2 - 100, HEIGHT // 2 + 180, 200, 60)
    pygame.draw.rect(SCREEN, GREEN, play_again_button, border_radius=10)
    button_text = MESSAGE_FONT.render("PLAY AGAIN", True, BLACK)
    SCREEN.blit(button_text, (play_again_button.centerx - button_text.get_width() // 2,
                              play_again_button.centery - button_text.get_height() // 2))
    return play_again_button
 
def reset_game():
    global ATTEMPTS, CURRENT_LETTERS, GAME_OVER, WON, SECRET_WORD
    ATTEMPTS = []
    CURRENT_LETTERS = ""
    GAME_OVER = False
    WON = False
...
while True:
    ...
    if GAME_OVER:
        ...
        play_again_button = show_play_again_button()
